...
const Update: React.FC = () => {
  ...
  const [newVersionUI, setNewVersionUI] =
    useState<newversion>(defaultNewVersion);
  const [curVersionUI, setCurVersionUI] = useState<version>(defaultVersion);
  const [newVersionSLAMNAV, setNewVersionSLAMNAV] =
    useState<newversion>(defaultNewVersion);
  const [curVersionSLAMNAV, setCurVersionSLAMNAV] =
    useState<version>(defaultVersion);
  const [newVersionSLAMNAV2, setNewVersionSLAMNAV2] =
    useState<newversion>(defaultNewVersion);
  const [curVersionSLAMNAV2, setCurVersionSLAMNAV2] =
    useState<version>(defaultVersion);
  ...
  async function updateSLAMNAV2(_version: string) {
    try {
      ...
      if (_version == undefined) {
        ...
        setCurVersionSLAMNAV2({
          prev_version: response.data.log.prev_version,
          version: response.data.log.new_version,
          date: response.data.log.date,
        });
  ...
  const RollbackDialog = () => {
    const [rollbackVersion, setRollbackVersion] =
      useState<newversion>(defaultNewVersion);
    const [rollbackVersions, setRollbackVersions] = useState<newversion[]>([
      defaultNewVersion,
    ]);
          ...
          <div className="col-12">
            <div className="flex flex-column md:flex-row align-items-center p-1 w-full">
              <Chip
                ...
                className="justify-content-center md:w-10rem shadow-2 font-bold"
              />
      ...
      >
        <div className="card flex justify-content-center">
          <Dropdown
            ...
            onChange={(e) => setRollbackVersion(e.value)}
            options={rollbackVersions}
            optionLabel="version"
            placeholder="Select a Rollback Version"
        ...
        <Button
          ...
          style={{ marginLeft: 10 }}
          onClick={() => {
            update(programRollback, rollbackVersion.version);
            setDisplayRollback(false);
          }}
        >
          Rollback
        ...
        >
          <div className="card">
            ...
            <div className="field grid">
              <label className="col-12 mb-2 md:col-2 md:mb-0">현재 버전</label>
            ...
            <div className="field grid">
              <label className="col-12 mb-2 md:col-2 md:mb-0">최신 버전</label>
              <div className="col-12 md:col-10">
                <InputText
                  type="text"
