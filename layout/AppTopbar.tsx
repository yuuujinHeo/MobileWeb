...
const AppTopbar = forwardRef<AppTopbarRef>((props, ref) => {
  ...
  useEffect(() => {
    ...
    const newPush = (
      ...
      if (
        ...
        if (href != '/login') {
          if (
            !confirm(
              '페이지를 이동하시면 저장되지 않은 내용이 사라집니다. 계속 진행하시겠습니까?'
            )
          ) {
            return;
          }
    ...
    console.log(originalPush, window.location, newPush);
  ...
  const getAvatar = () => {
    if (User.avatar == 'icon') {
      return <Avatar icon={User.source} shape="circle"></Avatar>;
    } else {
      //image
      return (
    ...
    if (User.avatar == 'icon') {
      ...
      return (
        <Avatar
          size="large"
          onClick={(e) => profile.current?.toggle(e)}
          image={Network.monitor + User.source}
          shape="circle"
        ></Avatar>
      );
    }
  ...
  const logout = async (_state: string) => {
    ...
    if (User?.state != 'guest' && User?.state != 'master') {
      const response = await axios.post(Network?.monitor + '/logout', {
    ...
    try {
      if (User?.state == '' || User?.state == 'user') {
        const response = await axios.get(
          Network?.monitor + '/user/login/' + User?.user_id
        );
  ...
  const SLAMContent = (
    <>
      <span
        style={{
          backgroundColor:
            Connection?.slamnav == true ? Color.good : Color.error,
        }}
  ...
  const TASKContent = (
    <>
      ...
      <span className="ml-2 font-medium">
        TASK :{' '}
        {Connection?.task == true ? taskState?.runningTaskName : 'Discon'}
      </span>
    </>
  );
  ...
  const TASKRunContent = (
    <>
      <span
        style={{
          backgroundColor: taskState?.running == true ? Color.good : Color.none,
        }}
        className="border-circle w-2rem h-2rem flex align-items-center justify-content-center"
      ></span>
      <span className="ml-2 font-medium">
  ...
  const RobotContent = (
    <>
      ...
      >
        <i
          className={
            Status?.condition.auto_state == 'move'
              ? 'pi pi-spin pi-spinner'
              : 'pi pi-spinner'
          }
          style={{ color: 'white' }}
  ...
  return (
    <div className="layout-topbar">
      ...
      >
        <p>
          페이지를 이동하시면 저장되지 않은 내용이 사라집니다. 계속
          진행하시겠습니까?
        </p>
      ...
      {Connection?.slamnav && (
        <Chip className="pl-0 pr-3 mr-2" template={RobotContent}></Chip>
      )}
      ...
      >
        {User?.state != 'guest' && User?.state != 'master' && (
          <div className="flex">
            <Chip
              icon="pi pi-clock"
              className={loginTime > 60 ? 'chip_normal' : 'chip_hurry'}
              label={
                String(Math.floor(loginTime / 60)).padStart(2, '0') +
                ':' +
                String(loginTime % 60).padStart(2, '0')
              }
            ></Chip>
            <Button
              label="연장"
        ...
        <OverlayPanel ref={profile} className="profile-panel">
          <div className="profile-panel-container">
            ...
            <h5 className="font-bold ">
              {User?.user_name == '' ? 'Unknown' : User?.user_name}
            </h5>
            {User?.state != 'guest' && User?.state != 'master' && (
              <div className="flex">
                <Chip
