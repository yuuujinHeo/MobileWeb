...
export const ChartCustom = ({ data, className }) => {
  ...
  const calcWidth = (index, width) => {
    ...
    const timeOver =
      (new Date(data[data.length - 1].time) - new Date(data[index].time)) /
      1000;
    const timeOver2 =
      (new Date(data[data.length - 1].time) - new Date(data[index + 1].time)) /
      1000;
    const timeIndex = Math.floor(
      (new Date(data[index + 1].time) - new Date(data[index].time)) / 1000
    );
    ...
    if (timeOver > timeAll) {
      ...
      return Math.floor((width * timeIndex) / timeAll) == 0
        ? 1
        : Math.floor((width * timeIndex) / timeAll);
    }
  };
